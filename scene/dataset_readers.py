...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, semantic_feature_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        foundation_model = os.path.basename(semantic_feature_folder).split(".")[0].split("_")[0]
        ...
        # if foundation_model =='langsplat':
        #     # dummy semantic feature
        #     semantic_feature = torch.zeros(512, height, width)
        # else:
...
def readColmapSceneInfo(path, foundation_model, images, eval, llffhold=8):
    ...
    semantic_feature_dir = foundation_model + "_features"
    ...
    print("Semantic feature shape: ", cam_infos[0].semantic_feature.shape)
...
def readCamerasFromTransforms(path, transformsfile, white_background, semantic_feature_folder, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames), desc="load images"):
            ...
            """ load SAM mask and CLIP feat. [OpenGaussian]
...
def readNerfSyntheticInfo(path, foundation_model, white_background, eval, extension=".png"):
    ...
    print("semantic_feature_folder:", semantic_feature_folder)
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, semantic_feature_folder, extension)
    """print("Reading Test Transforms")
    if os.path.exists(os.path.join(path, "transforms_test.json")):
        test_cam_infos = readCamerasFromTransforms(path, "transforms_test.json", white_background, semantic_feature_folder, extension)
    else:
