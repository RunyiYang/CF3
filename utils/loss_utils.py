...
def custom_loss(network_output, gt):
    custom_loss = torch.mul(gt, network_output)
    return custom_loss.sum()

def custom_loss2(network_output, gt):
    with torch.no_grad():
        norm_output = torch.norm(network_output, dim=0)[None, :, :].detach()
        print(norm_output.shape, network_output.shape)
    custom_loss = torch.mul(gt, network_output) / norm_output
    return custom_loss.sum()
