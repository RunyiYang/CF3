...
def parse_args():
    ...
    parser.add_argument("--input_path", type=str, default='room/iteration_33000')
...
class Quantization():
    def __init__(self, opt):
        ...
        if opt.sh_degree == 3:
            ...
            self.sh_dim = 3+24
        elif opt.sh_degree == 0:
            self.sh_dim = 3

        ply_path = os.path.join(opt.input_path, "feature_field.ply")
        # Create save directory if it doesn't exist
        os.makedirs(opt.save_path, exist_ok=True)
        # # Copy the ply file from input path to save path
        shutil.copy(os.path.join(opt.input_path, "point_cloud.ply"), os.path.join(opt.save_path, "point_cloud.ply"))
        shutil.copy(os.path.join(opt.input_path, "autoencoder.pth"), os.path.join(opt.save_path, "autoencoder.pth"))
        self.feats = read_ply_data(ply_path)
    ...
    def quantize(self):
        ...
        if only_vq_some_vector:
            ...
            print("top k: ", int(tensor_importance.shape[0] * (1-self.vq_ratio)))
